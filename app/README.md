...
The Shoal messenger UI — **Flutter** on top of the Rust `shoal-api` engine via
...
├───────────────────────────────┤
│ ShoalEngine (lib/engine.dart) │  thin Dart wrapper
├───────────────────────────────┤
│ flutter_rust_bridge (generated)│  lib/src/rust/  ← codegen output
├───────────────────────────────┤
│ app/rust  →  shoal-api (Rust)  │  ShoalApp: identity, contacts, chat
│                → shoal-client → the whole protocol (crypto stays in Rust)
└───────────────────────────────┘
```

The UI never sees a key: it calls `ShoalApp` (identity, contacts, `send`,
...
`app/rust` is the flutter_rust_bridge crate: it depends on `shoal-api` (this
workspace) and re-exports `ShoalApp` for the bridge to bind. See
`flutter_rust_bridge.yaml` for paths.

## Status

Scaffold: the Rust engine (`shoal-api`) is complete and tested; the Dart screens
(onboarding, chats, thread, add-contact) and theme are in place and call the
engine through `ShoalEngine`. Running `flutter_rust_bridge_codegen generate`
fills in `lib/src/rust/` and the app builds. **State persists across restarts**:
the engine exports its sessions, contacts, and history, and `ShoalEngine`
