...
const _seedKey = 'shoal.master_seed';
const _vaultKey = 'shoal.seed_vault'; // base64 password-encrypted seed
const _duressVaultKey = 'shoal.duress_vault'; // vault for the decoy seed
const _modeKey = 'shoal.mode'; // 'network' | 'relay:<addr>' | 'memory'
const _stateKey = 'shoal.state_blob'; // base64 sessions + contacts + history
const _decoyStateKey = 'shoal.decoy_state'; // separate state for the decoy account
const _notesKey = 'shoal.notes_blob'; // encrypted local-only notes (real account)
const _decoyNotesKey = 'shoal.decoy_notes'; // encrypted notes for the decoy account
...
const _proxyPassKey = 'shoal.proxy_pass';
const _proxyTorFirstKey = 'shoal.proxy_tor_first'; // chain order: Tor→SOCKS5 vs SOCKS5→Tor

/// The default Tor SOCKS5 endpoint (Orbot on Android).
const _torEndpoint = '127.0.0.1:9050';
const _favNodesKey = 'shoal.favorite_nodes'; // node ids the user starred
const _notifyKey = 'shoal.notifications'; // show a notification on new messages
const _nodeVerifiedKey = 'shoal.node_verified'; // completed the sync/verify wait
const _secureScreenKey = 'shoal.secure_screen'; // block screenshots (default on)
const _backgroundKey = 'shoal.background'; // 24/7 background service (default on)
const _disguiseKey = 'shoal.disguise'; // launcher icon/name: default|calculator|notes|weather
const _autoLockMinKey = 'shoal.autolock_minutes'; // 0 = off, else re-lock after N min
const _lockOnBgKey = 'shoal.lock_on_background'; // re-lock the moment the app backgrounds
const _wipeAttemptsKey = 'shoal.wipe_after_attempts'; // 0 = off, else wipe after N wrong unlocks
const _failedUnlocksKey = 'shoal.failed_unlocks'; // persisted wrong-password counter
...
/// Thrown by [ShoalEngineController.unlock] when the failed-attempt limit is hit
...
class ShoalEngineController extends ChangeNotifier {
  ShoalEngine? _engine;
  Timer? _pollTimer;
  Timer? _coverTimer;
  ...
  String get myShoalId => _engine?.myShoalId() ?? '';
  ...
  /// restore the same Shoal ID on another device. Requires the engine to be
  /// running (unlocked).
  ...
  /// Restore an identity from a 24-word recovery phrase (same seed → same Shoal
  ...
  Future<void> _start(Uint8List seed) async {
    ...
    if (_mode == 'network') {
      _engine = _anonReceive
          ? await ShoalEngine.newOnNetworkWithReceive(
              masterSeed: seed, bootstrap: _bootstrap, nodeListen: '0.0.0.0:0')
          : await ShoalEngine.newOnNetwork(masterSeed: seed, bootstrap: _bootstrap);
    } else if (_mode.startsWith('relay:')) {
      _engine =
          await ShoalEngine.newWithRelay(masterSeed: seed, relayAddr: _mode.substring(6));
    } else {
      _engine = ShoalEngine.newInMemory(masterSeed: seed);
    }
    _pollTimer?.cancel();
  ...
  List<ChatMessage> history(String shoalId) =>
      _engine?.history(shoalId: shoalId) ?? const [];
  ...
  ShoalEngine get _requireEngine {
    final engine = _engine;
    if (engine == null) {
  ...
  String safetyNumber(String shoalId) =>
      _requireEngine.safetyNumber(shoalId: shoalId);

  void addContact({
    required String name,
    required String shoalId,
    required Uint8List bundle,
  }) {
    _requireEngine.addContact(name: name, shoalId: shoalId, bundle: bundle);
    _persist();
    notifyListeners();
  }

  Future<void> send({required String shoalId, required String text}) async {
    await _requireEngine.send(shoalId: shoalId, text: text);
    _persist();
    notifyListeners();
  }

  /// Retry a message whose earlier send failed (shown with a failed marker).
  Future<void> resend({required String shoalId, required BigInt id}) async {
    await _engine?.resend(shoalId: shoalId, id: id);
    _persist();
    notifyListeners();
  }

  /// Edit one of our own sent messages, syncing the new text to the peer.
  Future<void> editMessage(String shoalId, BigInt id, String newText) async {
    await _engine?.editMessage(shoalId: shoalId, id: id, newText: newText);
    _persist();
    notifyListeners();
  }
  ...
  Future<void> deleteMessage(
    ...
    for (final m in history(shoalId)) {
      if (m.id == id && m.path.isNotEmpty) {
        await AttachmentStore.remove(m.path);
      }
    }
    await _engine?.deleteMessage(shoalId: shoalId, id: id, forBoth: forBoth);
    _persist();
    notifyListeners();
  }
  ...
  Future<void> sendAttachment({
    ...
    await _persistAttachment(engine, id, shoalId);
    notifyListeners();
  ...
  Future<void> _pumpUpload(ShoalEngine engine, BigInt id) async {
    try {
      var guard = 0;
      while (true) {
  ...
  Future<void> react(String shoalId, BigInt targetId, String emoji) async {
    await _engine?.react(shoalId: shoalId, targetId: targetId, emoji: emoji);
    _persist();
    notifyListeners();
  }
  ...
  Future<void> resendAttachment(String shoalId, ChatMessage m) async {
    final engine = _engine;
    if (engine == null) return;
    final plain = await attachmentBytes(m);
    if (plain == null) return;
    await engine.resendAttachment(shoalId: shoalId, id: m.id, bytes: plain);
    notifyListeners();
  ...
  Future<void> _drainAttachments(ShoalEngine engine) async {
  ...
  bool chatHasPassword(String shoalId) =>
      _engine?.chatHasPassword(shoalId: shoalId) ?? false;

  /// Whether this conversation is currently locked (needs its password).
  bool chatLocked(String shoalId) =>
      _engine?.chatLocked(shoalId: shoalId) ?? false;

  /// Set a password on this conversation (its history is sealed under it).
  Future<void> setChatPassword(String shoalId, String password) async {
    await _engine?.setChatPassword(shoalId: shoalId, password: password);
    _persist();
    notifyListeners();
  }

  /// Remove this conversation's password (it must be unlocked).
  Future<void> removeChatPassword(String shoalId) async {
    await _engine?.removeChatPassword(shoalId: shoalId);
    _persist();
    notifyListeners();
  }

  /// Unlock a locked conversation with its password (throws on a wrong one).
  Future<void> unlockChat(String shoalId, String password) async {
    await _engine?.unlockChat(shoalId: shoalId, password: password);
    _persist();
    notifyListeners();
  }

  /// Pin or unpin a chat (pinned chats sort to the top of the list).
  Future<void> setPinned(String shoalId, bool pinned) async {
    await _engine?.setPinned(shoalId: shoalId, pinned: pinned);
    _persist();
    notifyListeners();
  }

  /// Block or unblock a contact (a blocked contact's messages are dropped).
  Future<void> setBlocked(String shoalId, bool blocked) async {
    await _engine?.setBlocked(shoalId: shoalId, blocked: blocked);
    _persist();
    notifyListeners();
  }

  /// Move a chat one place up or down within its pinned group.
  Future<void> moveChat(String shoalId, {required bool up}) async {
    await _engine?.moveChat(shoalId: shoalId, up: up);
    _persist();
    notifyListeners();
  }

  /// Delete a conversation on this device only.
  Future<void> deleteChat(String shoalId) async {
    await _engine?.deleteChat(shoalId: shoalId);
    _persist();
    notifyListeners();
  }

  /// Delete a conversation for both sides (asks the peer to delete it too).
  Future<void> deleteChatForBoth(String shoalId) async {
    await _engine?.deleteChatForBoth(shoalId: shoalId);
    _persist();
    notifyListeners();
  }
  ...
  void markRead(String shoalId) {
    _engine?.markRead(shoalId: shoalId);
  }

  /// The disappearing-message timer for a chat, in seconds (0 = off).
  int disappearingSecs(String shoalId) =>
      _engine?.disappearingSecs(shoalId: shoalId) ?? 0;

  /// Set the disappearing-message timer for a chat (0 = off); synced to the peer.
  void setDisappearing(String shoalId, int secs) {
    _engine?.setDisappearing(shoalId: shoalId, secs: secs);
    notifyListeners();
  }
