...
//!
//! A mailbox envelope transports opaque `inner` bytes (`shoal-mailbox`); this
//! module defines what an Shoal client puts there. Because the inner bytes are
//! sealed to the recipient, including the sender's Shoal ID here preserves
...
use shoal_session::{InitialMessage, Message};
...
fn put_shoal_id(w: &mut Vec<u8>, id: &ShoalId) {
...
fn get_shoal_id(r: &mut Reader) -> Option<ShoalId> {
    let bytes = r.lp()?;
