//! # shoal-mix — the Shoal mixnet
//!
//! Turns [`shoal_net`]'s Sphinx routing into a real networked layer, adds a
...
//!   sender to the deposited message. Drop-in: an `ShoalClient`/`ShoalApp` uses
//!   it unchanged.
//!
...
use shoal_crypto::fill_random;
use shoal_mailbox::{Envelope, MailboxError, MailboxStore};
use shoal_net::loopix::exp_delay;
use shoal_net::rng::ChaChaRng;
use shoal_net::{
...
const POW_DOMAIN: &[u8] = b"shoal/node/pow/v1";
...
impl NodeDescriptor {
    ...
    pub fn pow_valid(&self) -> bool {
        let id_ok = shoal_crypto::sha256(&self.public)[..NODE_ID_LEN] == self.id;
...
pub fn send_cover(pool: &[NodeDescriptor], hops: usize) -> io::Result<()> {
    ...
    let mut rng = ChaChaRng::from_os();
    let hops = hops.clamp(1, shoal_net::MAX_HOPS);
    let packet = shoal_net::loopix::drop_cover(&hop_descs, hops, &mut rng)
...
pub const MAX_FETCH_SURBS: usize = (shoal_net::PAYLOAD_LEN - 1 - 8 - 4) / SURB_HEADER_LEN;
...
impl<P: MailboxStore> MixnetStore<P> {
    ...
    /// * `hops` — mixes before the exit, clamped to fit [`shoal_net::MAX_HOPS`].
    pub fn new(
        reader: P,
    ...
    fn provider_for(&self, recipient: &shoal_mailbox::RecipientKey) -> NodeDescriptor {
...
pub fn provider_index(view_public: &[u8; 32], num_providers: usize) -> usize {
    ...
    let h = shoal_crypto::sha256(view_public);
...
mod tests {
    ...
    fn mail_is_sharded_to_the_recipients_provider() {
        ...
        let recipient = shoal_mailbox::RecipientKey([9u8; 32]);
    ...
    fn anonymous_receive_over_surbs() {
        ...
        let envelope = shoal_mailbox::seal(&bob.view_public(), b"anonymous inbox!").unwrap();
        ...
        assert_eq!(
            shoal_mailbox::open(bob.view(), env).as_deref(),
