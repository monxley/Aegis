//! # shoal-mailbox — Phase 2
//!
//! Blind store-and-forward delivery for Shoal (Layer 4a of
//! [`SHOAL_PROTOCOL.md`](../../../SHOAL_PROTOCOL.md) §5). It ties the stealth
//! addressing of `shoal-identity` (recipient anonymity, G5) to a **sealed
...
//! `shoal-session`). Wrapping them here hides even the ratchet header and the
...
/// key type without depending on `shoal-identity` directly.
pub use shoal_identity::ViewPublicKey as RecipientKey;

/// The envelope AEAD binds the address and ephemeral so a relay cannot move a
/// ciphertext to a different address undetected.
const ENVELOPE_AAD_DOMAIN: &[u8] = b"shoal/mailbox/v1";

/// A fixed all-zero nonce is safe here: the envelope key is derived from a
/// fresh per-message ephemeral (see `shoal-identity`'s `envelope_key_*`), so a
