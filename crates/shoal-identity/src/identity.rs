//! Shoal identities, view keys, and the human-facing Shoal ID.
//!
...
//! the shareable [`ShoalId`] commits to `SHA-256(IK^sig)` (32 bytes) rather
...
use shoal_crypto::{fill_random, sha256, x25519::SecretKey};
...
impl ViewKeypair {
    ...
    pub fn matches(&self, ephemeral: &EphemeralPublic, address: &StealthAddress) -> bool {
        ...
        recomputed.view_tag == address.view_tag
            && shoal_crypto::ct_eq(&recomputed.addr_tag, &address.addr_tag)
    }
...
/// A full Shoal identity: view keypair, identity DH keypair, and ML-DSA
/// signing keypair.
pub struct Identity {
...
impl Identity {
    ...
    /// The shareable Shoal ID committing to `(IK, V, SHA-256(IK^sig))`.
...
/// Errors decoding an [`ShoalId`].
#[derive(Clone, Copy, PartialEq, Eq, Debug)]
pub enum ShoalIdError {
...
impl core::fmt::Display for ShoalIdError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let s = match self {
            ShoalIdError::BadPrefix => "missing 'shoal:' prefix",
            ShoalIdError::BadCharacter => "invalid base32 character",
            ShoalIdError::BadLength => "wrong decoded length",
            ShoalIdError::BadVersion => "unknown Shoal ID version",
            ShoalIdError::BadChecksum => "checksum mismatch (mistyped ID?)",
        };
        f.write_str(s)
    }
}

impl std::error::Error for ShoalIdError {}

const SHOAL_ID_PREFIX: &str = "shoal:";
const SHOAL_ID_VERSION: u8 = 1;
const CHECKSUM_LEN: usize = 4;
// version(1) + identity_dh(32) + view(32) + signing-key hash(32)
const PAYLOAD_LEN: usize = 1 + 32 + 32 + 32;

/// A shareable Shoal identity string: `shoal:` + base32(version ‖ IK ‖ V ‖
...
impl ShoalId {
    ...
    pub fn encode(&self) -> String {
        ...
        format!("{SHOAL_ID_PREFIX}{}", base32_encode(&framed))
    }

    /// Decode from a shareable `shoal:...` string, verifying the checksum.
    pub fn decode(s: &str) -> Result<ShoalId, ShoalIdError> {
        let body = s
            .strip_prefix(SHOAL_ID_PREFIX)
            .ok_or(ShoalIdError::BadPrefix)?;
        let framed = base32_decode(body)?;
        if framed.len() != PAYLOAD_LEN + CHECKSUM_LEN {
            return Err(ShoalIdError::BadLength);
        }
        let (payload, checksum) = framed.split_at(PAYLOAD_LEN);
        if payload[0] != SHOAL_ID_VERSION {
            return Err(ShoalIdError::BadVersion);
        }
        let expected = sha256(payload);
        if checksum != &expected[..CHECKSUM_LEN] {
            return Err(ShoalIdError::BadChecksum);
        }
...
const SAFETY_DOMAIN: &[u8] = b"shoal/safety-number/v1";
...
pub fn safety_number(a: &ShoalId, b: &ShoalId) -> String {
...
impl core::fmt::Display for ShoalId {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
...
fn base32_decode(s: &str) -> Result<Vec<u8>, ShoalIdError> {
    ...
    for ch in s.bytes() {
        let val = match ch {
            b'a'..=b'z' => ch - b'a',
            b'2'..=b'7' => ch - b'2' + 26,
            _ => return Err(ShoalIdError::BadCharacter),
        } as u32;
        buffer = (buffer << 5) | val;
        bits += 5;
        if bits >= 8 {
