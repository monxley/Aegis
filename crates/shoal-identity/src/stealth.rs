...
//! both:       σ  = HKDF(salt=∅, ikm=S, info="shoal/addr/v1" ‖ R ‖ V, 17)
//!             addr_tag = σ[0..16]  (relay storage key)
//!             view_tag = σ[16]     (1-byte fast-reject during scanning)
//! ```

use shoal_crypto::{fill_random, hkdf_expand, hkdf_extract, x25519::SecretKey};
...
const ADDR_INFO_PREFIX: &[u8] = b"shoal/addr/v1";
const ENVELOPE_INFO_PREFIX: &[u8] = b"shoal/envelope/v1";
...
/// σ = HKDF(∅, shared, "shoal/addr/v1" ‖ R ‖ V, 17) → (addr_tag, view_tag).
...
/// A per-message envelope key `K_env = HKDF(S, "shoal/envelope/v1" ‖ R ‖ V, 32)`,
