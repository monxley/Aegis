//! # shoal-identity — Phase 0
//!
//! Shoal identities, human-facing Shoal IDs, and **stealth addressing** for
//! recipient anonymity. This is the first crate of the Shoal messenger; it
//! implements Layer 1 of [`SHOAL_PROTOCOL.md`](../../../SHOAL_PROTOCOL.md),
...
//! SHA-256, HMAC/HKDF, ML-DSA-65) live in the `shoal-crypto` crate and are
//! verified against official RFC/FIPS test vectors.
//!
//! ## Quick tour
//!
//! ```
//! use shoal_identity::{Identity, stealth};
//!
//! // Bob publishes his Shoal ID; Alice decodes it.
//! let bob = Identity::generate();
//! let bob_id = bob.shoal_id().encode();
//! let bob_pub = shoal_identity::ShoalId::decode(&bob_id).unwrap();
//!
...
pub use identity::{safety_number, Identity, ShoalId, ShoalIdError, ViewKeypair};
pub use stealth::{
...
mod tests {
    ...
    fn safety_number_is_symmetric_stable_and_binds_keys() {
        let alice = Identity::from_secret_bytes(seed(1), seed(2), seed(3)).shoal_id();
        let bob = Identity::from_secret_bytes(seed(4), seed(5), seed(6)).shoal_id();
        let mallory = Identity::from_secret_bytes(seed(7), seed(8), seed(9)).shoal_id();
    ...
    fn decoded_shoal_id_addresses_the_real_recipient() {
        ...
        let shared = bob.shoal_id().encode();
        let bob_pub = ShoalId::decode(&shared).unwrap();
        let (address, ephemeral) = stealth::create(&bob_pub.view_public()).unwrap();
        assert!(bob.view().matches(&ephemeral, &address));
    }

    #[test]
    fn corrupted_shoal_id_fails_checksum() {
        ...
        match ShoalId::decode(&corrupted) {
            Err(ShoalIdError::BadChecksum) | Err(ShoalIdError::BadVersion) => {}
            other => panic!("expected checksum/version rejection, got {other:?}"),
        }
    }

    #[test]
    fn shoal_id_bad_prefix_is_rejected() {
        assert_eq!(ShoalId::decode("nope:abc"), Err(ShoalIdError::BadPrefix));
    }

    #[test]
    fn identity_signs_and_verifies() {
        use shoal_crypto::ml_dsa;
    ...
    #[test]
    fn shoal_id_binds_the_signing_key() {
        let bob = Identity::from_secret_bytes(seed(1), seed(2), seed(3));
        let id = bob.shoal_id();
    ...
    #[test]
    fn shoal_id_round_trip_preserves_the_signing_hash() {
        let id = Identity::from_secret_bytes(seed(10), seed(20), seed(30)).shoal_id();
        let decoded = ShoalId::decode(&id.encode()).unwrap();
