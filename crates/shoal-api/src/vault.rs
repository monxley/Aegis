...
const AAD: &[u8] = b"shoal-seed-vault-v1";
...
pub fn seal_secret(password: &str, secret: &[u8]) -> Vec<u8> {
    ...
    let mut key = [0u8; 32];
    shoal_crypto::pbkdf2_sha256(password.as_bytes(), &salt, ITERATIONS, &mut key);
...
mod tests {
    ...
    fn legacy_v1_blob_still_opens() {
        ...
        let mut key = [0u8; 32];
        shoal_crypto::pbkdf2_sha256(b"legacy", &salt, ITERATIONS_V1, &mut key);
