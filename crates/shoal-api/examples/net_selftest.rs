//! Network self-test: drive two throwaway identities through a **live** Shoal
...
//!
//! ```sh
//! cargo run -p shoal-api --example net_selftest -- 135.181.125.178:5078
//! ```
//!
...
fn main() {
    ...
    shoal_mix::set_pow_difficulty(20);

    let mut seed_a = [0u8; 32];
    let mut seed_b = [0u8; 32];
    shoal_crypto::fill_random(&mut seed_a);
    shoal_crypto::fill_random(&mut seed_b);

    println!("• discovering the network from {boot} …");
    let mut alice = match ShoalApp::create_on_network(seed_a.to_vec(), vec![boot.clone()]) {
        Ok(a) => a,
        Err(e) => {
    ...
    let mut bob = match ShoalApp::create_on_network(seed_b.to_vec(), vec![boot]) {
        Ok(b) => b,
        Err(e) => {
    ...
    alice
        .send(bob.my_shoal_id(), "selftest ping".into())
        .unwrap();
