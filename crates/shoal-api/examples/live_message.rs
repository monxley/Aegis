//! A real end-to-end run of the Shoal engine.
//!
//! Spins up a **live Ciphra blind server** in-process, creates two
//! [`ShoalApp`]s (the exact engine the Flutter UI drives) pointed at it,
//! exchanges identities, and sends a message each way over the real relay.
//! Every byte the server stores is an already-sealed Shoal envelope.
//!
//! Run it:  `cargo run -p shoal-api --example live_message`
...
fn spawn_blind_server() -> std::net::SocketAddr {
    ...
    let dir = std::env::temp_dir().join(format!("shoal-demo-{}", u64::from_le_bytes(rand)));
...
fn main() {
    ...
    let mut alice = ShoalApp::create_with_relay(vec![1u8; 32], addr.to_string())
        .expect("alice connects to relay");
    let mut bob = ShoalApp::create_with_relay(vec![2u8; 32], addr.to_string())
        .expect("bob connects to relay");
    println!("• Alice: {}", alice.my_shoal_id());
    println!("• Bob:   {}", bob.my_shoal_id());

    // They exchange Shoal IDs + prekey bundles out of band (paste / QR) and add
    ...
    let msg = "hello Bob — this is Shoal, end-to-end & post-quantum";
    alice
        .send(bob.my_shoal_id(), msg.into())
        .expect("alice sends");
    ...
    bob.send(alice.my_shoal_id(), reply.into())
        .expect("bob replies");
