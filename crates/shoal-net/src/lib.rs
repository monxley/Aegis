//! # shoal-net — Phase 3
//!
//! Network-layer anonymity for Shoal (Layer 4b of
//! [`SHOAL_PROTOCOL.md`](../../../SHOAL_PROTOCOL.md) §6), with the exact
...
//! the primitives in [`shoal_crypto`] (X25519 for the DH/blinding chain,
...
//!
//! ```
//! use shoal_net::{MixNode, SphinxPacket, ProcessedPacket, DEST_MARKER};
//!
...
use shoal_crypto::{chacha20, hmac_sha256, sha256};
...
/// rejected by [`SphinxPacket::seal`]. Sized to carry a full Shoal sealed
...
impl SphinxPacket {
    ...
    pub fn seal(path: &[Hop], message: &[u8]) -> Result<SphinxPacket, SphinxError> {
        ...
        shoal_crypto::fill_random(&mut ephemeral);
...
fn rho_key(s: &[u8; 32]) -> [u8; 32] {
    derive(b"shoal/sphinx/rho", s)
}
fn mu_key(s: &[u8; 32]) -> [u8; 32] {
    derive(b"shoal/sphinx/mu", s)
}
fn pi_key(s: &[u8; 32]) -> [u8; 32] {
    derive(b"shoal/sphinx/pi", s)
}
...
fn blind(alpha: &[u8; 32], s: &[u8; 32]) -> [u8; 32] {
    ...
    input.extend_from_slice(b"shoal/sphinx/blind");
...
fn lion_subkeys(pi: &[u8; 32]) -> [[u8; 32]; 4] {
    [
        derive(b"shoal/lioness/k1", pi),
        derive(b"shoal/lioness/k2", pi),
        derive(b"shoal/lioness/k3", pi),
        derive(b"shoal/lioness/k4", pi),
    ]
}
...
mod tests {
    ...
    #[test]
    fn an_shoal_sized_envelope_fits_one_packet() {
