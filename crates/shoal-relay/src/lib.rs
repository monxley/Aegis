...
//! turns Shoal's local mailbox into a real, deployable store-and-forward relay
//! (Layer 4a, `SHOAL_PROTOCOL.md` §5).
//!
...
//! - **At rest** the server stores exactly what it is handed — and Shoal only
//!   ever hands it *already sealed* envelopes ([`shoal_mailbox`]), so the relay
...
//! Swapping [`shoal_mailbox::InMemoryStore`] for a `CiphraStore` is the only
