...
//! is the relay, and two `ShoalClient`s hold a conversation through it — every
//! byte the server stores is an already-sealed Shoal envelope, so it learns
...
fn spawn_blind_server() -> std::net::SocketAddr {
    ...
    let dir = std::env::temp_dir().join(format!("shoal-ciphra-{}", u64::from_le_bytes(rand)));
...
fn a_conversation_flows_through_a_live_ciphra_relay() {
    ...
    bob.send(&mut relay, &alice.shoal_id(), b"hi alice over ciphra")
        .unwrap();
...
fn sweep_deletes_stored_messages_but_a_seq_cursor_survives_it() {
    ...
    alice
        .start_conversation(&mut store, &bob.shoal_id(), &bob.bundle(), b"hi")
        .unwrap();
    ...
    alice
        .send(&mut store, &bob.shoal_id(), b"after sweep")
        .unwrap();
