//! `shoal-relay-server` — run your own Shoal relay.
//!
//! A relay is a **Ciphra blind server**: it stores and serves sealed Shoal
...
//! point clients at it with `ShoalApp::create_with_relay("host:5077")`.
//!
//! ```text
//! shoal-relay-server                 # 0.0.0.0:5077, data in ./shoal-relay-data
//! shoal-relay-server -l 0.0.0.0:5077 -d /var/lib/shoal-relay
//! ```
//!
...
use shoal_mix::{DirectoryState, MailboxDeliver, MixService, NodeDescriptor};
use shoal_net::MixNode;
use shoal_relay::CiphraStore;

const DEFAULT_LISTEN: &str = "0.0.0.0:5077";
const DEFAULT_DATA_DIR: &str = "./shoal-relay-data";
...
fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(message) => {
            eprintln!("shoal-relay-server: error: {message}");
            ExitCode::FAILURE
        }
    }
}
...
fn run() -> Result<(), String> {
    ...
    println!("Shoal relay — a blind server that stores sealed envelopes it cannot read.");
...
fn start_mix_node(
    ...
    for b in bootstrap {
        match shoal_mix::discover(b.as_str()) {
            Ok(nodes) => directory.merge(&nodes),
            Err(e) => eprintln!("warning: bootstrap {b} unreachable: {e}"),
        }
    }

    shoal_mix::run_gossip(directory, desc, Duration::from_secs(30));
    Ok(())
}
...
fn load_or_create_mix_key(data_dir: &str) -> Result<[u8; 32], String> {
    ...
    if path.exists() {
        ...
        let mut seed = [0u8; 32];
        shoal_crypto::fill_random(&mut seed);
...
fn print_help() {
    println!(
        "shoal-relay-server — run your own Shoal node (blind mailbox + mixnet)

USAGE:
    shoal-relay-server [--listen <ADDR>] [--data <DIR>]
    shoal-relay-server --mix <ADDR> --advertise-mix <HOST:PORT> \\
