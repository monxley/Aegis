//! # shoal-session — Phases 1–1.6
//!
//! Session setup and message encryption for Shoal: the machinery that makes a
//! message unreadable even if intercepted, now *and* against a future quantum
//! adversary. Implements Layers 2–3 of the protocol
//! ([`SHOAL_PROTOCOL.md`](../../../SHOAL_PROTOCOL.md)), with the exact math in
...
//!
//! Zero third-party dependencies — all primitives come from [`shoal_crypto`].
//!
//! ```
...
/// `ShoalId` commitment) is the caller's responsibility.
...
mod tests {
    ...
    fn signature_from_a_different_key_fails_verification() {
        ...
        // specific identity via an ShoalId is tested in shoal-identity.)
        let mut bundle = bob_secrets(true).public_bundle();
