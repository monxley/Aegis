...
//! by checking it against an `ShoalId`'s committed hash (`shoal-identity`).
//!
...
//! phase wires these to a single [`shoal_identity::Identity`] so the same
//! identity keys back both the Shoal ID and the bundle.

use shoal_crypto::x25519::SecretKey;
use shoal_crypto::{ml_dsa, ml_kem, sha256};

const BUNDLE_DOMAIN: &[u8] = b"shoal/prekey-bundle/v1";
...
impl PrekeyBundle {
    ...
    /// *whose* key that is, check it against an `ShoalId` commitment.
    ...
    /// `SHA-256(identity_signing_public)` — the value an [`ShoalId`] commits
...
impl PrekeySecrets {
    ...
    pub fn generate() -> Self {
        ...
        let mut opk = [0u8; 32];
        let mut sig = [0u8; 32];
        shoal_crypto::fill_random(&mut ik);
        shoal_crypto::fill_random(&mut spk);
        shoal_crypto::fill_random(&mut opk);
        shoal_crypto::fill_random(&mut sig);
        Self::build(
            ik,
            spk,
