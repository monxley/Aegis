//! End-to-end authenticity (G8): the full chain from a shareable Shoal ID to a
//! verified, identity-bound prekey bundle.
//!
//! `shoal-session` signs and verifies a bundle's *self-consistency* (the
//! signature matches the advertised signing key), and `shoal-identity`'s
//! `ShoalId` commits to `SHA-256(IK^sig)`. Binding the two — "this bundle's
//! signing key really is the one behind Bob's Shoal ID" — is what defeats a
...
use shoal_session::{establish_initiator, establish_responder, PrekeySecrets};
...
#[test]
fn shoal_id_binds_the_bundle_signing_key() {
    let (identity, secrets) = bob_identity_and_secrets();
    let shoal_id = identity.shoal_id();
    let bundle = secrets.public_bundle();

    // The bundle is internally consistent...
    assert!(bundle.verify());
    // ...its identity DH key matches the ID...
    assert_eq!(bundle.identity_dh, shoal_id.identity_dh_public());
    // ...and its signing key is exactly the one Bob's Shoal ID commits to.
    assert!(shoal_id.verify_signing_key(&bundle.identity_signing_public));
    assert_eq!(bundle.signing_key_hash(), shoal_id.signing_key_hash());
}

#[test]
fn mitm_bundle_from_a_different_key_is_caught_by_the_binding() {
    // Bob's real Shoal ID, shared out of band.
    let (bob, _) = bob_identity_and_secrets();
    let bob_id_string = bob.shoal_id().encode();
    let bob_id = ShoalId::decode(&bob_id_string).unwrap();
    ...
    // ...but its signing key is NOT the one Bob's Shoal ID commits to, so the
...
fn a_verified_bound_bundle_yields_a_working_session() {
    ...
    let bob_id = bob_identity.shoal_id();
