...
case "${PUBLIC_HOST:-}" in
  YOUR_HOST|your.host|node2.host|host|example.com|example.org|my.host|changeme|node.example|seed.example|node2.example)
...
case "${BOOTSTRAP:-}" in
  SEED_HOST*|seed.host*|seed.example*|node2.host*|bootstrap.example*)
