# Shoal Messaging Protocol — Design (v0 draft)

**Shoal is an anonymous, end-to-end-encrypted, post-quantum messenger built
...
> the Double Ratchet, CryptoNote stealth addresses, Sphinx, Loopix). Shoal
> writes *those* from scratch on top of Ciphra's primitives. Shoal does **not**
...
- **The relay operator itself** — runs an Shoal/Ciphra node. Blind by
...
- **Endpoint compromise at runtime** — if the adversary owns the device while
  Shoal is unlocked, plaintext and keys are in memory. Out of scope, same as
...
- **Global passive adversary defeating a busy mixnet** — Loopix (§6) raises the
  ...
  *impossibility*.
- **Anonymity-set of one** — anonymity is only as large as the crowd of Shoal
...
- **We do not use the Monero/BTC blockchain as a message bus.** Putting data
  on-chain costs a fee per message, is slow, size-limited and public. Shoal
...
An Shoal identity is a bundle of keys, generated locally, never registered with
any authority:
...
`v` is the secret view scalar; `V` its public point. An **Shoal ID** shown to
...
3. secret   = HKDF-SHA256(shared, info = "shoal/addr/v1")
...
secret'   = HKDF-SHA256(shared', "shoal/addr/v1")
...
- **Sender-pays**: a tiny Monero payment or burn proof attached to first
  contact. This is the *only* place a blockchain earns its keep in Shoal, and
  it is optional. Not a message bus — an anti-Sybil toll.

### 2.5 Optional extension — full CryptoNote (Ed25519) addressing

If Shoal later wants one-time addresses that are *also signable keys* (e.g. to
...
upgraded to its post-quantum form **PQXDH**. Shoal reuses the exact primitive
...
Signal's PQ ratchet direction (PQ3 / "SPQR"), Shoal periodically performs an
...
holds no keys (ADR-0003). Shoal models a message as a row:
...
> multi-node, blind, replicated mailbox" layer is **already implemented**. Shoal
...
Cover traffic and mixing trade **bandwidth and latency** for anonymity. Shoal
...
option, Shoal can route **every** outbound TCP — both the mixnet dials and the
...
## 8. Mapping to Ciphra crates — what exists vs. what Shoal adds

| Shoal need | Status | Where |
|---|---|---|
...
| **Stealth addressing (DH-only)** | ✅ done | `shoal-identity` |
| **PQXDH session setup** | ✅ done | `shoal-session` |
| **PQ Double Ratchet (ongoing ML-KEM)** | ✅ done | `shoal-session` |
| **ML-DSA-signed prekey bundles (G8)** | ✅ done | `shoal-session` + `shoal-identity` |
| **Sealed-sender envelope + mailbox** | ✅ done | `shoal-mailbox` |
| **Sphinx onion routing (LIONESS payload)** | ✅ done | `shoal-net` |
| **Loopix mixing + cover traffic** | ✅ done | `shoal-net` |
| **Live Ciphra blind-server relay** | ✅ done | `shoal-relay` (Ciphra `RemoteStorage`) |
| **One-identity client API** | ✅ done | `shoal-client` |
| Ed25519 / CryptoNote signable one-time keys | ⏳ optional/future | would extend `ciphra-crypto` (§2.5) |

**Read of the table:** every *primitive* Shoal needs already existed in Ciphra
and is test-vector-verified; the Shoal crates re-implement the small subset not
publicly exported. Shoal is protocol-assembly work, not cryptographic-primitive
...
shoal/
├── crates/
│   ├── shoal-crypto/     # zero-dep primitives (x25519, ml_kem, ml_dsa, aead, …)
│   ├── shoal-identity/   # Shoal ID, keypairs, signing, stealth addressing (§2)
│   ├── shoal-session/    # PQXDH handshake (§3) + ongoing-PQ Double Ratchet (§4)
│   ├── shoal-mailbox/    # sealed-sender envelope + blind MailboxStore (§5)
│   ├── shoal-net/        # Sphinx onion routing (LIONESS) + Loopix mixing (§6)
│   ├── shoal-relay/      # CiphraStore: MailboxStore over a live Ciphra server
│   └── shoal-client/     # ShoalClient — one identity, one API over all layers
├── SHOAL_PROTOCOL.md     # this document
└── docs/CRYPTO_MATH.md   # the exact mathematics
```

`shoal-relay` depends on Ciphra's `ciphra-net` (`RemoteStorage`) to reach a live
...
- **Phase 0 — identity.** ✅ `shoal-identity`: keygen, Shoal ID encoding, DH-only
  stealth address derive + scan with view tags (§2), ML-DSA signing.
- **Phase 1 — sessions.** ✅ `shoal-session`: PQXDH from signed prekey bundles
  (§3) + ongoing-PQ Double Ratchet (§4). Delivers G1–G4, G8.
- **Phase 2 — delivery.** ✅ `shoal-mailbox` (sealed-sender envelopes, blind
  `MailboxStore`) + `shoal-relay` (`CiphraStore` over a **live Ciphra blind
  server**, tested end-to-end). Delivers G5, G6.
- **Phase 3 — network anonymity.** ✅ `shoal-net`: Sphinx onion routing with a
  non-malleable LIONESS payload (§6.1), and Loopix Poisson mixing + cover
  traffic (§6.2). Delivers G7.
- **Integration.** ✅ `shoal-client`: one identity, one messenger API over all
  layers.
...
Each phase is independently tested and leaves Shoal in a working state.
